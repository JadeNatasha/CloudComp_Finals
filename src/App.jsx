...
export default function App() {
  ...
  const [userSession, setUserSession] = useState(null); // Stores logged in account metadata
  ...
  const fetchProducts = () => {
    // Add a random timestamp to the end of the URL to bypass cache
  ...
  // --- GLOBAL SYNC FIX ---
  // Attaching this to window so SellerPortal can call it anywhere
  ...
  if (!isAuthenticated) {
    return (
      <div style={{ display: 'flex', minHeight: '100vh', fontFamily: '"Segoe UI", Tahoma, Geneva, Verdana, sans-serif', background: '#fff' }}>
        ...
        {/* ⚪ RIGHT ACCOUNT ACCESSIBILITY INPUT FORM SECTION */}
        <div style={{ flex: 1.2, display: 'flex', flexDirection: 'column', justifyContent: 'center', alignItems: 'center', padding: '40px', position: 'relative' }}>
          ...
          {/* 📋 VIEW A: CUSTOMER STOREFRONT LOGIN VIEW PANEL */}
          {authView === 'login' && (
          ...
          {/* 📝 VIEW C: UNIFIED SIGN UP ACTIONS AREA */}
          {authView === 'signup' && (
            <form onSubmit={handleSignUpSubmit} style={{ width: '100%', maxWidth: '380px', display: 'flex', flexDirection: 'column', gap: '16px' }}>
              ...
              <div style={{ display: 'flex', justifyContent: 'center', gap: '20px', margin: '4px 0' }}>
                <label style={{ color: '#4a3f3f', fontWeight: '600', fontSize: '0.95rem', cursor: 'pointer' }}>
                  <input type="radio" name="role" value="customer" checked={signupRole === 'customer'} onChange={() => setSignupRole('customer')} style={{ marginRight: '6px' }} />
                  I am a Customer
                </label>
                <label style={{ color: '#b06000', fontWeight: '600', fontSize: '0.95rem', cursor: 'pointer' }}>
                  <input type="radio" name="role" value="seller" checked={signupRole === 'seller'} onChange={() => setSignupRole('seller')} style={{ marginRight: '6px' }} />
                  I am a Seller
                </label>
              </div>
          ...
          {/* VIEW D: SELLER DASHBOARD */}
          {currentView === 'seller_dashboard' && (
            <SellerDashboard 
              onLogout={handleLogout} 
              refreshStorefront={fetchProducts} // <--- This connects them!
            />
          )}
  ...
  // 🏪 APP RENDER ENTRYWAY: Runs automatically after user is successfully validated
  return (
    <div className="storefront-root-wrapper">
      ...
      <header className="premium-store-header">
        ...
        <div className="header-icon-actions" style={{ alignItems: 'center' }}>
          ...
          <button 
            ...
            onClick={() => { setIsFavoritesOpen(true); setIsCartOpen(false); }}
          ...
          <button 
            className="nav-action-icon-btn" 
            onClick={() => { setIsCartOpen(true); setIsFavoritesOpen(false); }}
            title="Open Shopping Bag Drawer"
          >
            <i className="fa-solid fa-basket-shopping"></i>
            {cartCount > 0 && <span className="icon-badge-indicator">{cartCount}</span>}
          </button>
          
          {/* USER ACCOUNT DROPDOWN FRAME */}
          <div style={{ position: 'relative' }}>
            <button 
              className="navbar-user-account-btn"
              style={{
                cursor: 'pointer',
                display: 'flex',
                alignItems: 'center',
            ...
            {isProfileDropdownOpen && (
              ...
              >
                ...
                {userSession?.role === 'customer' && (
                  <>
                    <button 
                      style={{ width: '100%', textAlign: 'left', padding: '12px 20px', background: 'none', border: 'none', cursor: 'pointer', display: 'flex', alignItems: 'center', gap: '12px', fontSize: '0.9rem', color: '#4a3f3f' }}
                    ...
                    <button 
                      style={{ width: '100%', textAlign: 'left', padding: '12px 20px', background: 'none', border: 'none', cursor: 'pointer', display: 'flex', alignItems: 'center', gap: '12px', fontSize: '0.9rem', color: '#4a3f3f' }}
                      onClick={() => { setCurrentView('orders'); setIsProfileDropdownOpen(false); }}
                    >
                ...
                {userSession?.role === 'seller' && (
                  <button 
                    style={{ width: '100%', textAlign: 'left', padding: '12px 20px', background: 'none', border: 'none', cursor: 'pointer', display: 'flex', alignItems: 'center', gap: '12px', fontSize: '0.9rem', color: '#b06000', fontWeight: '600' }}
                    onClick={() => { setCurrentView('seller_dashboard'); setIsProfileDropdownOpen(false); }}
                  >
                ...
                <div style={{ borderTop: '1px solid #f5ebeb', marginTop: '8px', paddingTop: '8px' }}>
                  <button 
                    style={{ width: '100%', textAlign: 'left', padding: '12px 20px', background: 'none', border: 'none', cursor: 'pointer', display: 'flex', alignItems: 'center', gap: '12px', fontSize: '0.9rem', color: '#e05a5a', fontWeight: '600' }}
      ...
      {/* 3. DYNAMIC GATEWAY WORKSPACE HUB */}
      <main className="storefront-main-workspace">
        
        {/* VIEW A: PUBLIC STOREFRONT DISPLAY SCREEN */}
        {currentView === 'store' && (
          <>
              ...
              <div className="summer-promo-hero-banner">
                <div className="banner-editorial-content">
                  ...
                  <div style={{ display: 'flex', justifyContent: 'flex-start', width: '100%', marginTop: '12px' }}>
                    <button 
                      style={{ 
                        cursor: 'pointer', backgroundColor: '#4a3f3f', color: '#ffffff', border: 'none', borderRadius: '50px', padding: '14px 32px', fontSize: '0.95rem', fontWeight: '500', display: 'inline-flex', alignItems: 'center', justifyContent: 'center', gap: '8px', boxShadow: '0 4px 12px rgba(0,0,0,0.08)'
                      }}
                      onClick={() => {
                        const target = document.querySelector('.horizontal-category-strip');
        ...
        {/* VIEW B: PROFILE CREDS CONTROL WORKSPACE */}
        {currentView === 'profile' && (
          <div style={{ maxWidth: '960px', margin: '0 auto', padding: '0 20px' }}>
            ...
            <div style={{ background: '#fff', borderRadius: '20px', border: '1px solid #f2e6e6', padding: '40px', boxShadow: '0 4px 24px rgba(74, 63, 63, 0.03)' }}>
              ...
              {/* Shipping Addresses Row Section */}
        ...
        {/* VIEW C: LIVE DATABASE TRACKING ORDER ENGINE ROWS */}
        {currentView === 'orders' && (
      ...
      {/* 🛒 SIDE SLIDE-OUT CART DRAWER PANEL */}
      ...
      {/* 💖 SIDE SLIDE-OUT FAVORITES DRAWER PANEL */}
